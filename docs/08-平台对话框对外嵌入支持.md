...
`bridgeActions` 在创建 Chat Session 时被快照。业务前端新增、重命名或延迟注册 Page Action 后，必须刷新页面或重建 Chat Session，再做运行时联调；否则会出现平台目录已有 action，但当前 session 仍提示 `page action is not registered` 的假故障。
...
Embed SSE 以 `message.completed` 表示本轮回复结束，**没有** `done` 事件。
...
`apiBase` 与网关代理（二选一，见下文「Chat SDK apiBase」）：
...
当前 `eafChat` SDK 会请求 `${apiBase}/api/embed/chat/sessions` 与 `${apiBase}/api/embed/chat/sessions/{sessionId}/messages...`（见 `ai-admin-front/src/sdk/eafChat.ts`）。因此：

| 部署模式 | `apiBase` 配置 | 浏览器最终请求 |
| --- | --- | --- |
| **直连 ReachAI** | ReachAI 平台 origin，例如 `http://localhost:18603` | `<origin>/api/embed/**` |
| **经业务网关代理** | 仍应配置为 ReachAI 可达的 `/api/embed/**` 前缀；业务网关必须把 **`/api/embed/**`** 转发到 ReachAI `/api/embed/**` | 例如 `https://gateway.example.com/api/embed/chat/sessions` |

**错误示例**：`apiBase: '/api/reachai/embed'` 会变成 `/api/reachai/embed/api/embed/chat/sessions`。

如果业务网关只提供 `/api/reachai/embed/**` 重写到 ReachAI，当前 SDK **不能**直接写 `apiBase: '/api/reachai/embed'`；需要额外增加 `/api/embed/**` 网关路由，或后续调整 SDK 路径策略。token broker 路径（如 `/api/reachai/embed-token`）与 Chat API 的 `apiBase` 是两套地址，不要混用。

页面上下文 `pageKey` / `pageInstanceId` / `route` / `origin` 应在 token exchange、session create 与 Page Action 三处保持一致，便于 Workflow 按 `pageKey` 路由。
...
页面助手 bridge 内部可使用结构化 `error.code/message`；回传 Embed Page Action Result API（`POST /api/embed/chat/sessions/{sessionId}/page-actions/{requestId}/result`）时，当前平台 DTO 的 `error` 字段为 **字符串**。SDK/模板应在边界映射为字符串，或待 Embed API DTO 升级后再传对象。回传成功状态推荐 `SUCCESS`；SDK 内部的 `FAILED` / `CANCELLED` / `TIMEOUT` 等需在回传前映射为平台可接受的 `status` 字符串。
...
响应（`ApiResult` 包装；token broker 读取 `data.token` / `data.expiresIn`，字段名为 `sessionHint` 而非 `metadata`）：
...
响应（`ApiResult` 包装，读取 `data.sessionId`）：
