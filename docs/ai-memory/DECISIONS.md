...
新系统和核心系统优先通过 `ai-spring-boot-starter` 主动注册。平台扫描 Controller/OpenAPI 是存量和低改造场景的补充路径。
...
## ReachAI Branding Versus EAF Technical Identity

ReachAI 是产品品牌。`eaf.*` 配置、`X-EAF-*` header、`Eaf*` 类名、运行时路径和 Maven artifact 是技术身份和兼容边界。品牌文案可以改，技术身份不要顺手改。
