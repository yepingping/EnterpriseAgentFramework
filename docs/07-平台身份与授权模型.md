...
响应（`ApiResult` 包装，业务字段在 `data` 下读取，不要只读顶层 `token` / `expiresIn`）：
...
请求体（`pageKey` 应与 token exchange 使用同一稳定页面键，便于 Workflow 按 `pageKey` 路由）：
...
响应（`ApiResult` 包装，业务字段在 `data` 下读取，不要只读顶层 `sessionId`）：
...
Embed SSE 以 `message.completed` 表示本轮回复结束，**没有** `done` 事件；不要按平台内部 ChatController 的 `done` 事件解析。
